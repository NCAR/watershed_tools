...
def reproject_raster(inraster, outraster, dest_crs, resampling_method):
    ...
    with rio.open(inraster) as src:
        ...
        kwargs.update(
            {"crs": dest_crs, "transform": transform, "width": width, "height": height}
        )
...
def reproject_basin_shapefile(inShapefile, outShapefile, destination_crs):
    driverName = (
        "ESRI Shapefile"  # can later be upgraded to work with geopackages (eg 'GPKG')
    )
    driver = ogr.GetDriverByName(driverName)

    spatialref = destination_crs.to_string()  # in wkt format
    ogr2ogr.main(
        ["", "-f", driverName, "-t_srs", spatialref, outShapefile, inShapefile]
    )  # note: main is expecting sys.argv, w/ 1st arg as scriptname, hence ""
    return
...
def rasterize_gru_vector(
    ...
    in_gdf[[gruNo_fieldname, gruId_fieldname]].to_csv(
        gruNo_gruId_txt, sep=",", index=False
    )
    ...
    with rio.open(outraster, "w+", **meta) as out:
        ...
        shapes = (
            (geom, value)
            for geom, value in zip(in_gdf["geometry"], in_gdf[gruNo_fieldname])
        )
        ...
        burned = rio.features.rasterize(
            shapes=shapes, out=out_arr, fill=nodatavals, transform=out.transform
        )
...
def rasterize_vector(invector, infield, infield_dtype, refraster, outraster):
    ...
    with rio.open(outraster, "w+", **meta) as out:
        ...
        # this is where we create a generator of geom, value pairs to use in rasterizing
        shapes = (
            (geom, value) for geom, value in zip(in_gdf["geometry"], in_gdf[infield])
        )
...
def crop_raster(inraster, invector, outraster):
    ...
    with rio.open(inraster) as src:
        if src.crs == in_gdf.crs:  # check projection consistency
            ...
            print(
                "Crop failed because input raster and vector are in different projections."
            )
    ...
    for i in range(nx):  # latitude (north -> south)
        for j in range(ny):  # longitude (west -> east)
            if dem[i, j] != nodatavals:
                ...
                # dz/dx and dz/dy
                dzdx = ((dem_c + 2 * dem_f + dem_i) - (dem_a + 2 * dem_d + dem_g)) / (
                    8 * pixelSizeX
                )
                dzdy = ((dem_g + 2 * dem_h + dem_i) - (dem_a + 2 * dem_b + dem_c)) / (
                    8 * pixelSizeY
                )
    ...
    with rio.open(inraster) as ff:
        ...
        with rio.open(inraster) as ff:
            ...
            transform = ff.transform * ff.transform.scale(
                (ff.width / data.shape[-1]), (ff.height / data.shape[-2])
            )
...
def classify_raster(
    inraster, bound_raster, classif_trigger, bins, class_outraster, value_outraster
):
    """
...
def polygonize_raster(inraster, outvector, attrb_field, attrb_field_dtype):
    ...
    with rio.Env():
        ...
        with fiona.open(
            ...
            schema={
                "properties": [(attrb_field, attrb_field_dtype)],
                "geometry": "Polygon",
            },
        ) as dst:
...
def define_hru(
    ...
    for irow, row in hru_gpd_disv.iterrows():
        ...
        if (
            gruNo_current == gruNo_before
        ):  # if the current gruNo is the same as before, cumulate hru num.
        ...
        gruName = (
            corr_df[corr_df[gruNo_field] == int(gruNo_current)]
            .reset_index()
            .loc[0, gruName_field]
        )
        hru_gpd_disv.loc[irow, hruName_field] = str(gruName) + str(
            hru_num_per_gru + 1
        ).zfill(
            2
        )  # two-digit HRU#
        ...
        for jfield, field in enumerate(fieldname_list):
            ...
            hru_gpd_disv.at[irow, field] = int(
                hru_str[field_str_start : field_str_start + field_str_len]
            )

    # change dtypes of name and class fields to be int64
    hru_gpd_disv[hruName_field] = pd.to_numeric(
        hru_gpd_disv[hruName_field], errors="coerce"
    )
    hru_gpd_disv[gruName_field] = pd.to_numeric(
        hru_gpd_disv[gruName_field], errors="coerce"
    )
...
def eliminate_small_hrus_dominant(
    hru_vector,
    hru_area_thld,
    gruNo_field,
    gruName_field,
    ...
    max_hru_str_len = max(
        [len(str(max(max_hru_num))), 2]
    )  # str_len min is 2 (e.g, 01,02,...)
    ...
    for irow, row in in_gpd_disv.iterrows():
        ...
        in_gpd_disv.at[irow, hruName_field] = int(
            str(in_gpd_disv.loc[irow, gruName_field]) + hru_num_str
        )
    ...
    rasterize_vector(
        hru_vector_disv, hruNo_field, hruNo_field_dtype, refraster, hru_raster_disv
    )
    return


# Eliminate small HRUs by merging with the largest neighbor HRU within the same GRU.
def eliminate_small_hrus_neighbor(
    hru_vector,
    hru_thld_type,
    hru_thld,
    gruNo_field,
    ...
    for gru in grus:
        ...
        if sum(gru_df.is_valid == False) > 0:  # check validity
            print(
                "Topology validity problems found, may cause problems in further processing"
            )
            print("STOP")
        dom_hru_idx = gru_df[
            hruArea_field
        ].idxmax()  # the most dominant HRU index in gru_df
        dom_hru = gru_df.loc[
            dom_hru_idx, hruName_field
        ]  # the most dominant HRU hruName in gru_df
        ...
        while row_num > 1 and elim_num != 0 and iter_num < 10:
            ...
            for irow, row in elim_df.iterrows():
                ...
                nbhds_idx = gru_df[
                    gru_df.geometry.touches(row["geometry"])
                ].index.tolist()
                ...
                if len(nbhds_idx) > 0:
                    ...
                    for field in fieldname_list:
                        in_gpd_disv.at[
                            in_gpd_disv[hruName_field] == target_hruName, field
                        ] = gru_df.loc[larg_nbhd_idx, field]
                    in_gpd_disv.at[
                        in_gpd_disv[hruName_field] == target_hruName, hruName_field
    ...
    pbar.close()
    print(
        "      -- info: no neighbors found for %d hrus, used dominant gru type instead"
        % no_neighbors_count
    )
    ...
    print(
        "    done: eliminated %d hrus out of original %d"
        % (num_orig_hrus - len(in_gpd_disv), num_orig_hrus)
    )
...
def zonal_statistic(
    ...
    **kwargs,
    ...
    if attr_SizeX <= ref_SizeX:
        ...
        attr_raster_resample = os.path.join(
            os.path.dirname(attr_raster),
            os.path.basename(attr_raster).split(".")[0] + "_resample.tif",
        )
    ...
    in_raster = os.path.join(
        os.path.dirname(invector), os.path.basename(invector).split(".")[0] + "tmp.shp"
    )
    ...
    for field_value in unique_field_value:
        ...
        if isinstance(metric, str):
            if metric == "mean":
                ...
                attr_smask = np.where(
                    attr_smask == 0.0, np.nan, attr_smask
                )  # exclude flat grids (flat aspect=0)
            ...
            in_gpd.loc[
                in_gpd[infield] == field_value, output_column_prefix
            ] = zonal_value
            output_array[smask] = zonal_value
        else:
            print(
                "Invalid metric string. Please choose from 'mean', 'median', 'max', 'min'."
            )

    # change astype as the same as attr_raster dtype
    in_gpd[output_column_prefix] = in_gpd[output_column_prefix].astype(
        out_meta["dtype"]
    )
...
def merge_shp_spatial_join(src1, src2, dst, merge_attr):
    ...
    if src1_gdf.crs != src2_gdf.crs:
        print(
            f"Projection (crs) of {src1} and {src2} are not the same! Proceeding with merge."
        )
