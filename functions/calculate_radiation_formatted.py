...
def process_command_line():
    ...
    parser.add_argument(
        "epsg", help="EPSG integer to assign coordinate systems and transformations."
    )
...
def calculate_Sw(latitude, elev, slope, aspect, DOY):
    ...
    declin = (23.45 * np.pi / 180.0) * np.sin(
        2 * np.pi * ((284 + DOY) / 365.0)
    )  # Earth's declination [rad]
    d_squared = 1 / (
        1 + 0.033 * np.cos(DOY * 2 * np.pi / 365)
    )  # Eqn 2, earth sun distance
    a = np.sin(declin) * np.cos(latitude) * np.sin(slope) * np.cos(aspect) - np.sin(
        declin
    ) * np.sin(latitude) * np.cos(
        slope
    )  # Eqn 11a, constant
    b = np.cos(declin) * np.cos(latitude) * np.cos(slope) + np.cos(declin) * np.sin(
        latitude
    ) * np.sin(slope) * np.cos(
        aspect
    )  # Eqn 11b, constant
    ...
    quad[
        quad < 0
    ] = 0.0001  # Limit quadratic function to greater than 0 as per step A.4.i.
    sin_w1 = (a * c - b * np.sqrt(quad)) / (b**2 + c**2)  # Eqn 13a
    sin_w1[sin_w1 > 1] = 1  # Limit sin_w1 to between 1 and -1 as per step A.4.iii.
    sin_w1[
        sin_w1 < 1 * -1
    ] = -1  # Limit sin_w1 to between 1 and -1 as per step A.4.iii.
    w1 = np.arcsin(sin_w1)
    ...
    w1_24 = w1
    w1_24[(cos_ws1 <= cos_w1) & (cos_w1 < 0.001)] = w1[
        (cos_ws1 <= cos_w1) & (cos_w1 < 0.001)
    ...
    w1_24 = np.maximum(
        w1_24, ws1
    )  # Sunrise [rad], Step A.2.vi #Element-wise maximum of array elements.
    ...
    sin_w2[
        sin_w2 < 1 * -1
    ] = -1  # Limit sin_w2 to between 1 and -1 as per step A.4.iii.
    w2 = np.arcsin(sin_w2)
    ...
    w2_24 = w2
    w2_24[(cos_ws2 <= cos_w2) & (cos_w2 < 0.001)] = w2[
        (cos_ws2 <= cos_w2) & (cos_w2 < 0.001)
    ...
    cos_theta = (
        np.sin(declin) * np.sin(latitude) * np.cos(slope) * (w2_24 - w1_24)
        - np.sin(declin)
        * np.cos(latitude)
        * np.sin(slope)
        * np.cos(aspect)
        ...
        * np.sin(slope)
        * np.sin(aspect)
        * (np.cos(w2_24) - np.cos(w1_24))
    )  # Eqn 5

    # Calculate 24H Extraterrestrial Radiation
    Ra_24 = (
        S * cos_theta / d_squared
    )  # 24 Hour extraterrestrial radiation [W/m2]. Eqn 6.
    ...
    e = (
        0.6108 * np.exp(17.27 * AirTemp / (AirTemp + 237.3)) * RH
    )  # Vapor Pressure as per ASCE_EWRI (2005) [kPa]. Eqn 7.
    P = (
        101.3 * ((293 - 0.0065 * elev) / 293) ** 5.36
    )  # Atmospheric Pressure as per ASCE-EWRI (2005) [kPa]. Eqn 34.
    ...
    cond_flat = slope == 0
    sin_Beta_24H[cond_flat] = (
        2 * (g[cond_flat] ** 2) * ws2[cond_flat]
        + 4 * g[cond_flat] * h[cond_flat] * np.sin(ws2[cond_flat])
        + (h[cond_flat] ** 2) * (ws2[cond_flat] + 0.5 * np.sin(2 * ws2[cond_flat]))
    ) / (2 * (g[cond_flat] * ws2[cond_flat] + h[cond_flat] * np.sin(ws2[cond_flat])))

    # Eqn 17, Clearness index for direct beam radiation [-]
    K_Bo_24H[cond_flat] = 0.98 * np.exp(
        (-0.00146 * P[cond_flat] / (Kt * sin_Beta_24H[cond_flat]))
    ...
    K_Bo = 0.98 * np.exp(
        (-0.00146 * P / (Kt * sin_Beta)) - 0.075 * (W / sin_Beta) ** 0.4
    )  # Eqn 17, Direct beam clearness index [-]
    ...
    Ra_H_24 = (S / d_squared) * (
        np.sin(declin) * np.sin(latitude) * (ws2 - ws1)
        + np.cos(declin) * np.cos(latitude) * (np.sin(ws2) - np.sin(ws1))
    )  # Eqn 5,ET radiation for horizontal surface [W/m2]
    sin_Beta_H = (
        2 * (g**2) * ws2
        + 4 * g * h * np.sin(ws2)
        + (h**2) * (ws2 + 0.5 * np.sin(2 * ws2))
    ) / (
        2 * (g * ws2 + h * np.sin(ws2))
    )  # Eqn 26, Angle of sun above horizon for horizontal surface [W/m2]
    K_Bo_H = 0.98 * np.exp(
        (-0.00146 * P / (Kt * sin_Beta_H)) - 0.075 * (W / sin_Beta_H) ** 0.4
    )  # Eqn 17, Direct radiation index [-]
    ...
    # Fractions
        ...
        0.75 + 0.25 * np.cos(slope) - 0.5 * slope / np.pi
    )  # Eqn 32, Sky view factor for isotropic conditions [-]
    fb = (K_Bo / K_Bo_H) * (
        Ra_24 / Ra_H_24
    )  # Eqn 34, Ratio of expected direct beam radiation on slope to radiation on horizontal
    fia = (1 - K_Bo_H) * (
        1 + ((K_Bo_H / (K_Bo_H + K_Do_H)) ** 0.5) * np.sin(slope / 2) ** 3
    ) * fi + fb * K_Bo_H  # Eqn 33, Sky view factor for anisotropic conditions [-]
    ...
    R_So_24H[cond_nan] = (K_Bo_H[cond_nan] + K_Do_H[cond_nan]) * Ra_H_24[
        cond_nan
    ]  # Eqn 23, Clear sky solar radiation for horizontal surface [W/m2]
    del cond_nan
...
if __name__ == "__main__":
    ...
    asp_ma = (
        np.deg2rad(asp_ma) - np.pi
    )  # Surface aspect redefined so that 0 degrees is south and pi/2 is west [rad]
    ...
    Sw_DOY_value = Sw_DOY.filled(
        fill_value=nodatavals
    )  # return copy with masked values filled with a given value
